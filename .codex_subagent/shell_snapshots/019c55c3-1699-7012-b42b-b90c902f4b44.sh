...
codex () {
	...
	local project_name="${cwd#$repo_root/}"
	project_name="${project_name%%/*}"
	local project_path="$repo_root/$project_name"
	if [[ -z "$project_name" || ! -d "$project_path" ]]
	then
		command codex "$@"
		return $?
	fi
	codex__ensure_serena_project "$project_name" "$project_path" "$cwd"
	codex__ensure_project_codex_config "$project_path"
	codex__ensure_profile "$project_name"
	local project_codex_config="$project_path/.codex.toml"
	local uvx_path
	...
	local IFS=$'\n'
	local -a overrides=("${(@f)$(codex__build_overrides "$project_codex_config" "$project_path" "$uvx_path")}")
	local override_entry
	for override_entry in "${overrides[@]}"
	do
		[[ -n "${override_entry// }" ]] || continue
		config_overrides+=("-c" "$override_entry")
	done
	echo "codex using profile '$project_name' (serena project: $project_path)"
	command codex -C "$project_path" --profile "$project_name" "${config_overrides[@]}" "$@"
}
codex__build_overrides () {
	local project_codex_config="$1"
	...
	then
		overrides_output=$(python3 - "$project_codex_config" "$project_path" "$uvx_path" <<'PY'
import copy
import sys
...
def to_inline(value):
    ...
    if isinstance(value, list):
        return "[" + ",".join(to_inline(item) for item in value) + "]"
    if isinstance(value, dict):
        return (
            "{"
            + ",".join(f"{key}={to_inline(val)}" for key, val in value.items())
            + "}"
        )
    raise TypeError(f"Unsupported type in MCP config: {type(value)!r}")
...
if project_config.exists():
    raw_config = project_config.read_text()
config_data = {}
if raw_config.strip():
    if tomllib is None:
        print(
            f"Warning: TOML support unavailable in python3; ignoring {project_config}",
            file=sys.stderr,
        )
    else:
        try:
            config_data = tomllib.loads(raw_config)
        except Exception as exc:  # pragma: no cover - defensive
            print(f"Failed to parse {project_config}: {exc}", file=sys.stderr)
            sys.exit(1)
...
final_servers = OrderedDict()
if "serena" in mcp_servers and isinstance(mcp_servers["serena"], dict):
    serena_entry = copy.deepcopy(serena_default)
    deep_update(serena_entry, mcp_servers["serena"])
else:
    serena_entry = serena_default

if "serena" in mcp_servers:
    for name, payload in mcp_servers.items():
        if name == "serena":
            final_servers[name] = serena_entry
        elif isinstance(payload, dict):
            final_servers[name] = payload
else:
    final_servers["serena"] = serena_entry
    for name, payload in mcp_servers.items():
        if isinstance(payload, dict):
            final_servers[name] = payload

if "serena" not in final_servers:
    final_servers["serena"] = serena_entry

for name, payload in final_servers.items():
    print(f"mcp_servers.{name}={to_inline(payload)}")
PY
)
		python_status=$?
	else
		python_status=1
		echo "Warning: python3 not found; skipping project Codex overrides from $project_codex_config" >&2
	fi
	if (( python_status == 0 )) && [[ -n "$overrides_output" ]]
	then
		printf '%s\n' "$overrides_output"
		return 0
	fi
	if (( python_status != 0 ))
	then
		echo "Warning: falling back to default Serena MCP configuration for '$project_path'." >&2
	fi
	codex__default_serena_override "$uvx_path" "$project_path"
}
codex__default_serena_override () {
	local uvx_path="$1"
	local project_path="$2"
	printf 'mcp_servers.serena={command="%s",args=["--refresh","--from","git+https://github.com/oraios/serena","serena","start-mcp-server","--context","codex","--project-from-cwd","--open-web-dashboard","False"],optional=true,autostart=false,startup_timeout_ms=30000,request_timeout_ms=15000}' "$uvx_path"
}
codex__ensure_profile () {
	...
	then
		command rg --fixed-strings --quiet "[profiles.${project_name}]" "$config_file" || profile_exists=0
	else
		command grep -Fqx "[profiles.${project_name}]" "$config_file" || profile_exists=0
	fi
	if (( profile_exists == 0 ))
	then
		{
			printf '\n[profiles.%s]\n' "$project_name"
			printf 'model = "gpt-5.2-codex"\n'
			printf 'model_reasoning_effort = "medium"\n'
		} >> "$config_file"
		echo "Added default Codex profile for '$project_name' to $config_file"
	fi
}
codex__ensure_project_codex_config () {
	local project_path="$1"
	local project_codex_config="$project_path/.codex.toml"
	if [[ ! -f "$project_codex_config" ]]
	then
		cat <<'EOF' > "$project_codex_config"
...
# ]
EOF
		echo "Created Codex project config at $project_codex_config"
	fi
}
codex__ensure_serena_project () {
	local project_name="$1"
	local project_path="$2"
	local cwd="$3"
	[[ "$cwd" == "$project_path" ]] || return 0
	if ! command -v uvx > /dev/null 2>&1
	then
		echo "Warning: uvx not found; skipping Serena indexing for '$project_name'." >&2
		return 0
	fi
	local project_config="$project_path/.serena/project.yml"
	if [[ ! -f "$project_config" ]]
	then
		...
		echo "Created Serena project config at $project_config"
	fi
	if [[ ! -d "$project_path/.serena/index" ]]
	then
		echo "Initializing Serena MCP project in $project_path"
		if ! (
				builtin cd "$project_path" && command uvx --refresh --from git+https://github.com/oraios/serena serena project index
			)
		then
			local serena_language=""
			if command -v python3 > /dev/null 2>&1 && [[ -f "$project_config" ]]
			then
				serena_language=$(python3 - "$project_config" <<'PY'
import sys
from pathlib import Path
...
PY
			...
			if [[ -z "${serena_language// }" ]]
			then
				echo "Serena project language is missing; scanning project files to infer one (Codex will launch soon)..."
				codex__infer_serena_language "$project_path" "$project_config" || true
			fi
		fi
	fi
}
codex__infer_serena_language () {
	local project_path="$1"
	local project_config="$2"
	if ! command -v python3 > /dev/null 2>&1
	then
		echo "Warning: python3 not available; cannot infer Serena project language automatically." >&2
		return 1
	fi
	local inference_output=""
	if ! inference_output=$(python3 - "$project_path" <<'PY'
import os
import sys
...
ignore_dirs = {".git", ".serena", "node_modules", "vendor", "venv", ".venv", ".env", "env", "__pycache__", "build", "dist", ".idea", ".vscode"}
counts = {language: 0 for language in language_exts}
...
for current_root, dirs, files in os.walk(project_root):
    ...
    for name in files:
        ...
        ext = ext.lower()
        for language, extensions in language_exts.items():
...
PY
	...
	then
		echo "Warning: failed to infer Serena project language automatically." >&2
		return 1
	fi
	local language_guess=""
	local language_count="0"
	language_guess=$(printf "%s\n" "$inference_output" | sed -n '1p')
	language_count=$(printf "%s\n" "$inference_output" | sed -n '2p')
	if [[ -z "$language_guess" ]]
	then
		language_guess="markdown"
	fi
	if [[ -z "$language_count" ]]
	then
		language_count="0"
	fi
	echo "Detected candidate language '${language_guess}' (matched ${language_count} files)."
	local reply
	read -r "?Update $project_config with language '${language_guess}'? [Y/n] " reply
	if [[ -z "$reply" || "$reply" == [Yy]* ]]
	then
		python3 - "$project_config" "$language_guess" <<'PY'
import sys
from pathlib import Path
...
for idx, line in enumerate(lines):
    if line.strip().startswith("language:"):
        lines[idx] = f"language: {language}"
        break
else:
    lines.append(f"language: {language}")
config_path.write_text("\n".join(lines) + "\n")
PY
		echo "Updated $project_config with language '${language_guess}'. Re-running Serena index..."
		if ! (
				builtin cd "$project_path" && command uvx --refresh --from git+https://github.com/oraios/serena serena project index
			)
		then
			echo "Warning: Serena indexing still failing; please adjust $project_config manually." >&2
		fi
	else
		echo "Skipped updating Serena project language; Codex may run without Serena indexing." >&2
	fi
}
colors () {
	emulate -L zsh
...
comfyui_run () {
	local base="$HOME/ComfyUI"
	builtin cd "$base" || return
	source venv/bin/activate
	PYTORCH_ENABLE_MPS_FALLBACK=1 python main.py --force-fp16 --use-split-cross-attention --enable-manager --preview-method auto --lowvram --disable-xformers --cpu-vae
}
comfyui_update () {
	local base="$HOME/ComfyUI"
	builtin cd "$base" || return
	source venv/bin/activate
	comfyui_update_custom_nodes
	builtin cd "$base" || return
	git fetch && git pull
	pip install -q -r requirements.txt
	pip install -q -r manager_requirements.txt
}
comfyui_update_custom_nodes () {
	local base="$HOME/ComfyUI/custom_nodes"
	builtin cd "$base" || return
	local req
	git fetch && git pull
	...
	do
		[[ -f "$req" ]] || continue
		builtin cd "${req%/requirements.txt}" || continue
		pip install -q -r requirements.txt
		builtin cd ..
	done
}
...
git-init-perso () {
	git config user.name "Pezzos"
	git config user.email "alexandrepezzotta@gmail.com"
	git config push.default simple
	git config --unset alias.review 2> /dev/null
	git config --unset alias.draft 2> /dev/null
	echo "Git user set to alexandrepezzotta@gmail.com"
	echo "Configured git push for standard GitHub workflow"
}
git_commits_ahead () {
...
# exports 60
export BUNDLED_DEBUGPY_PATH=/Users/alexandrepezzotta/.cursor/extensions/ms-python.debugpy-2025.18.0-darwin-arm64/bundled/libs/debugpy
export CODEX_HOME=/Users/alexandrepezzotta/repos/PezzosCode/.codex_subagent
export CODEX_MANAGED_BY_NPM=1
export COLORTERM=truecolor
export COMMAND_MODE=unix2003
export CONDA_DEFAULT_ENV=base
export CONDA_EXE=/Users/alexandrepezzotta/miniconda3/bin/conda
export CONDA_PREFIX=/Users/alexandrepezzotta/miniconda3
export CONDA_PROMPT_MODIFIER='(base) '
export CONDA_PYTHON_EXE=/Users/alexandrepezzotta/miniconda3/bin/python
export CONDA_SHLVL=1
export CPATH=/usr/local/include
export CURSOR_TRACE_ID=185039f04e35499f95c902b99214d30c
export F=18
export GIT_ASKPASS=/Applications/Cursor.app/Contents/Resources/app/extensions/git/dist/askpass.sh
export HOME=/Users/alexandrepezzotta
export LANG=C.UTF-8
export LESS=-R
export LIBRARY_PATH=/usr/local/lib
export LOGNAME=alexandrepezzotta
export LSCOLORS=Gxfxcxdxbxegedabagacad
export LS_COLORS='di=1;36:ln=35:so=32:pi=33:ex=31:bd=34;46:cd=34;43:su=30;41:sg=30;46:tw=30;42:ow=30;43'
export MAKEFLAGS='F=18'
export MAKELEVEL=1
...
export PYTHONPATH=/Users/alexandrepezzotta/repos/PezzosCode/tools/serena/solidlsp_override:/Users/alexandrepezzotta/repos/PezzosCode/tools/serena/solidlsp_override:/Users/alexandrepezzotta/repos/PezzosCode/tools/serena/solidlsp_override:
export RESUME_MODE=sync
export SDKROOT=/Library/Developer/CommandLineTools/SDKs/MacOSX.sdk
export SERENA_LSP_CONFIG_PING_DIR=/tmp
export SHELL=/bin/zsh
export SSH_AUTH_SOCK=/private/tmp/com.apple.launchd.wQyuwwNVxM/Listeners
export TERM=xterm-256color
export TERM_PROGRAM=vscode
export TERM_PROGRAM_VERSION=2.4.31
export TMPDIR=/var/folders/0k/n_ybf_014_z5pynwc4c87jvw0000gn/T/
export USER=alexandrepezzotta
export USER_ZDOTDIR=/Users/alexandrepezzotta
export VSCODE_DEBUGPY_ADAPTER_ENDPOINTS=/Users/alexandrepezzotta/.cursor/extensions/ms-python.debugpy-2025.18.0-darwin-arm64/.noConfigDebugAdapterEndpoints/endpoint-af0b3efe09e05396.txt
export VSCODE_GIT_ASKPASS_EXTRA_ARGS=''
export VSCODE_GIT_ASKPASS_MAIN=/Applications/Cursor.app/Contents/Resources/app/extensions/git/dist/askpass-main.js
export VSCODE_GIT_ASKPASS_NODE='/Applications/Cursor.app/Contents/Frameworks/Cursor Helper (Plugin).app/Contents/MacOS/Cursor Helper (Plugin)'
export VSCODE_GIT_IPC_AUTH_TOKEN=fea9468cc437fffa9bad1fe41c113bd1e117b2a591b1ece1b870c4ed115a06ae
export VSCODE_GIT_IPC_HANDLE=/var/folders/0k/n_ybf_014_z5pynwc4c87jvw0000gn/T/vscode-git-6791e61625.sock
export VSCODE_INJECTION=1
export VSCODE_PROFILE_INITIALIZED=1
export XPC_FLAGS=0x0
export XPC_SERVICE_NAME=0
export ZDOTDIR=/Users/alexandrepezzotta
export ZSH=/Users/alexandrepezzotta/.oh-my-zsh
export _CE_CONDA=''
export _CE_M=''
export _CONDA_EXE=/Users/alexandrepezzotta/miniconda3/bin/conda
export _CONDA_ROOT=/Users/alexandrepezzotta/miniconda3
