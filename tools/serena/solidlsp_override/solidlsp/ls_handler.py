...
class SolidLanguageServerHandler:
    ...
    def on_request(self, method: str, cb: Callable[[Any], Any]) -> None:
        ...
        if method == "workspace/configuration":
            prev = self.on_request_handlers.get(method)
            prev_name = getattr(prev, "__qualname__", repr(prev))
            prev_module = getattr(prev, "__module__", "unknown")
            new_name = getattr(cb, "__qualname__", repr(cb))
            new_module = getattr(cb, "__module__", "unknown")
            stack = "".join(traceback.format_stack(limit=6))
            log.info(
                "Registering workspace/configuration; prev=%s prev_module=%s new=%s new_module=%s handler_id=%s stack=%s",
                prev_name,
                prev_module,
                new_name,
                new_module,
    ...
    def _request_handler(self, response: StringDict) -> None:
        ...
        if not handler:
            if method == "workspace/configuration":
                # Last-resort fallback to avoid MethodNotFound errors during init races.
                try:
                    if isinstance(params, dict) and "items" in params:
                        ...
                        result = [{}]
                    log.warning(
                        "workspace/configuration missing handler; returning empty config. handlers=%s handler_id=%s",
                        sorted(self.on_request_handlers.keys()),
                        id(self),
                    )
        ...
        try:
            if method == "workspace/configuration":
                try:
                    ...
                    log.warning(
                        "workspace/configuration handler failed; returning empty config instead. handler=%s err=%s",
