...
class TestPcFeature(unittest.TestCase):
    ...
    def test_commit_worktree_changes_uses_pc_role_commit_script(self):
        with mock.patch.object(
            self.pc_feature,
            "get_status_paths",
            return_value=["docs/02-features/01-workflow-hardening/reporter-log.md"],
        ):
            run_calls = []
            ...
            with mock.patch.object(
                ...
                self.pc_feature.commit_worktree_changes(
                    "/tmp/worktree",
                    "reporter",
                    "WI-20260214-05",
                    "docs/02-features/01-workflow-hardening",
        ...
        self.assertEqual(run_calls[0][0], "tools/pc-role-commit")
        self.assertIn("--role", run_calls[0])
        self.assertIn("reporter", run_calls[0])
        self.assertIn("--work-item-id", run_calls[0])
        self.assertIn("WI-20260214-05", run_calls[0])
        self.assertIn("--allow", run_calls[0])

    def test_commit_worktree_changes_surfaces_pc_role_commit_failure_detail(self):
        stderr_capture = io.StringIO()
        with mock.patch.object(
            self.pc_feature,
            "get_status_paths",
        ...
        self.assertIn("failed to commit changes for reporter", message)
    ...
    def test_select_resume_work_item_id_skips_legacy_bootstrap_entry(self):
        newest = "WI-20260206-03"
        content = (
            "## Execution Log\n\n"
            f"### {newest} - Work item execution\n\n"
            "- Date: 2026-02-06\n"
    ...
    def test_repair_missing_entry_sections_inserts_required_headers(self):
        work_item_id = "WI-20260206-09"
        content = (
            "## Execution Log\n\n"
            f"### {work_item_id} - Work item execution\n\n"
            "- Date: 2026-02-06\n"
            "- Scope / tasks covered:\n"
            "- Planner:\n"
            "- Plan Reviewer:\n"
        ...
        repaired, inserted = self.pc_feature.repair_missing_entry_sections(
            content,
            work_item_id,
        )
        self.assertIn("Patch", inserted)
        self.assertIn("Test Results", inserted)
        self.assertIn("Reporter Review", inserted)
        self.assertIn("#### Patch", repaired)
        self.assertIn("#### Test Results", repaired)
        self.assertIn("#### Reporter Review", repaired)
    ...
    def test_missing_prompt_without_templates_mentions_bootstrap_remediation(self):
        with tempfile.TemporaryDirectory() as tmp_dir:
            root = Path(tmp_dir)
            prompts_dir = root / "prompts"
            prompts_dir.mkdir(parents=True, exist_ok=True)
            original_root = self.pc_feature.ROOT
            original_prompts_dir = self.pc_feature.PROMPTS_DIR
            self.pc_feature.ROOT = root
            self.pc_feature.PROMPTS_DIR = prompts_dir
            stderr_capture = io.StringIO()
            try:
                with self.assertRaises(SystemExit):
                ...
                self.pc_feature.ROOT = original_root
                self.pc_feature.PROMPTS_DIR = original_prompts_dir
        message = stderr_capture.getvalue()
        self.assertIn("missing prompt template", message)
        self.assertIn("re-run bootstrap in this repo", message)
        self.assertIn("restore prompt file(s) under", message)
    ...
    def test_environment_lock_reporter_fail_is_normalized_before_retry_loop(self):
        class StopMain(RuntimeError):
            pass
        ...
        with tempfile.TemporaryDirectory() as tmp_dir:
            ...
            def fake_codex_exec(prompt: str, **kwargs) -> str:
                ...
                if "Review changes for scope and completeness" in prompt:
                    return (
                        ...
                        "Docs/logs updated: docs/02-features/01-workflow-hardening/reporter-log.md\n"
                        "File/Path: docs/02-features/01-workflow-hardening/dev-tasks.md\n"
                        "Check: Reporter runtime commit persistence.\n"
                        "Evidence: Updated reporter log, but commit blocked: fatal: unable to create '.git/index.lock': Operation not permitted in sandbox.\n"
                        "Expected fix: use scripted commit path.\n"
            ...
            self.assertIn(
                "Reporter environment-lock FAIL normalized to PASS",
                dev_tasks,
            )
    ...
    def test_is_environment_lock_only_reporter_failure_classifier(self):
        ...
        self.assertTrue(
            self.pc_feature.is_environment_lock_only_reporter_failure(
                environment_feedback
            )
        )
        ...
        actionable_feedback = (
            ...
            "Docs/logs updated: reporter-log.md\n"
            "Check: Reporter handoff completeness.\n"
            "Evidence: Reporter Review is still pending and commit hit '.git/index.lock' in sandbox.\n"
            "Expected fix: Populate Reporter Review section.\n"
        )
        self.assertFalse(
            self.pc_feature.is_environment_lock_only_reporter_failure(
