...
Use `prompts/<role>.md` and task variants like `prompts/<role>-<task>.md` (for example `plan-reviewer-gate`, `patcher-apply`) for role-specific instructions.
If a prompt file is missing, copy the matching template from `tools/templates/prompts/` and rerun the step (prompt loading is file-based only).
