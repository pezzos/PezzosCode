...
- Date: 2026-02-11
- Scope: resume-routing guardrails in `tools/pc-feature`, deterministic anti-hardcode resume fixtures in `tests/test_pc_feature.py`, and compacted evidence for this work item.

## Commands Executed

- `make feature F=17-resume-in-progress-tickets` -> blocked: `pc-feature: run from main branch only`.
- `tools/offload-proxy/pp python -m pytest tests/test_pc_feature.py::TestPcFeature` -> FAIL (`1 failed`, `118 passed`): env isolation regression in `test_parse_resume_mode_defaults_to_auto`.
...
- Offload pointer ids:
  ...
  - pytest runs: none (output stayed inline; no offload pointer generated)
