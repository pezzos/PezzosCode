...
## 2026-02-16 - Reporter retry cap still escalated to manual decision options for metadata-only variant

- **ID:** BUG-20260216-02
- **Status:** Fixed
- **Source:** User report (`make feature F=02` in `/Users/alexandrepezzotta/repos/Agenda-Assistant`, worktree `/Users/alexandrepezzotta/repos/Agenda-Assistant-02-mandatory-human-review-before-apply-patcher`)
- **Summary:** Reporter failures using execution-state/validation-log wording (`still records Outcome: needs replan ... while validation-log records PASS`) could bypass metadata-drift normalization as `scope_gap`; once retry cap was reached, `pc-feature` emitted decision-option escalation text instead of deterministically applying closeout repair.
- **Fix:** Expanded metadata-drift markers for the observed wording family and changed reporter retry-cap handling in `tools/pc-feature` to automatically apply deterministic closeout metadata repair (Option A) and rerun once; removed decision-options messaging from this retry-cap path and added deterministic terminal messaging after the rerun.
- **Validation:** `tools/offload-proxy/pp python3 -m unittest discover -s /Users/alexandrepezzotta/repos/PezzosCode/tests -p 'test_pc_feature.py' -k 'is_metadata_drift_only_reporter_failure_classifier'` (PASS, offload id `ff54520eb41a16677c9d2e2fc45533519a89fbdc9fb068da141805c0296b7a63`); `tools/offload-proxy/pp python3 -m unittest discover -s /Users/alexandrepezzotta/repos/PezzosCode/tests -p 'test_pc_feature.py' -k 'classify_reporter_failure_reason'` (PASS, offload id `a626407facf1102b512cc38f00f6e7d889aa6280a5c03d1722ac6d1c4eb08dc5`); `tools/offload-proxy/pp python3 -m unittest discover -s /Users/alexandrepezzotta/repos/PezzosCode/tests -p 'test_pc_feature.py' -k 'reporter_retry_cap_auto_repair_reruns_once_without_decision_options'` (PASS, offload id `123ffa564b34c76650916d85ef9f5179841df799d50f3c83a89f9792419c42fc`); `tools/offload-proxy/pp python3 -m unittest discover -s /Users/alexandrepezzotta/repos/PezzosCode/tests -p 'test_pc_feature.py'` (PASS, offload id `5331f1a5636557770f9c1f9e53cbcdc2831b3fb7bda1dc5ca2103983f7aa017a`).

## 2026-02-16 - Missing touched-test parity gate and retry-cap terminal status closure gap

- **ID:** BUG-20260216-03
- **Status:** Fixed
- **Source:** User report (`make feature F=08`, `WI-20260216-02` in `/Users/alexandrepezzotta/repos/Agenda-Assistant-08-output-offload-workflow-patcher`)
