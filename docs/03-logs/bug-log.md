...
## 2026-02-09 - Final commit failed on missing `.tmp` pathspec

- **ID:** BUG-20260209-03
- **Status:** Fixed
- **Source:** Internal feature run (`make feature F=13`)
- **Summary:** Final commit crashed in `pc-commit` when `git add -- .tmp` ran while `.tmp` did not exist.
- **Fix:** Filtered ephemeral allow paths in `pc-feature` to existing/tracked entries and hardened `pc-commit` to skip missing `--allow` paths before restaging.
- **Validation:** `python3 -m unittest tests.test_pc_feature` (`89` tests, PASS).
